...
namespace Obj
{
    ...
    public class ObjGeometryProcessor {
        ...
        private Color[] SetColors()
        {
            ...
            foreach (var pair in triangles)
            {
                ...
                if (primType == "triangle")
                {
                    ...
                    {
                        colors[pair.Value[i] - 2] = gradient.Evaluate(DataProcess.datList[0][id]);
                        colors[pair.Value[i] - 1] = gradient.Evaluate(DataProcess.datList[0][id]);
                        colors[pair.Value[i]] = gradient.Evaluate(DataProcess.datList[0][id]);
                        id++;
                    }
                }
                else
        ...
        private void FinishMeshProcessing(List<MeshData> resultsAppendList)
        {
            ...
            string[] primType = new string[triangles.Count];
            id = 0;
            foreach (var pair in triangles)
            {
                if (pair.Value.Count < 4)
                {
                    primType[id] = "triangle";
                }
                else if (pair.Value[0] == pair.Value[4] || pair.Value[2] == pair.Value[5])
                {
                    primType[id] = "patch";
